...
import { extractRelevantFields, processApiSpec } from '../lib/core/openapi-parser';
...
describe('fixtures', () => {
  fs.readdirSync(fixturesDir).forEach((caseName) => {
    ...
    test(`should generate correct mock for ${caseName}`, async () => {
      ...
      const config: Config = fs.existsSync(configPath) ? { ...baseConfig, ...(await import(configPath)) } : baseConfig;
      ...
      const generatedOutput = organizedApiData.map((endpoint) => {
        ...
        const templateGenerator = new TemplateGenerator(EXPORT_LANGUAGE.JS, isESM);

        return generateMockDataFileContent(generatedMocks, templateGenerator, endpoint, EXPORT_LANGUAGE.JS);
      });

      const expectedOutput = fs.readFileSync(outputPath, 'utf-8');

      const formattedGeneratedOutput = await formatCode(generatedOutput.join('\n').trim());
